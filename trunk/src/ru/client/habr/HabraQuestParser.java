...
public final class HabraQuestParser {
	...
	public HabraQuest parseQuestFromList() {
		...
		String questData = new String(mData.substring(startPosition, 
				endPosition)) + "</div></div>";
		...
		lastIndex += 17;
		quest.tags = new String(questData.substring(lastIndex, 
				lastIndex = questData.indexOf("</ul>", lastIndex)));   	
		...
		String rs = questData.substring(
				lastIndex = (questData.indexOf('>', (questData.indexOf("mark\">", 
						lastIndex) + 6)) + 1),
				lastIndex = questData.indexOf('<', lastIndex));
		quest.rating = (rs.charAt(0) == '-' ? -1 : 1);
		rs = "0" + rs.substring(1);
		...
		quest.date = new String(questData.substring(
				lastIndex = (questData.indexOf("<span>", 
						questData.indexOf("<div class=\"published\">", lastIndex)) + 6),
				lastIndex = questData.indexOf('<', lastIndex)));
		...
		String favs = questData.substring(
				lastIndex = (questData.indexOf('>', 
						questData.indexOf("<div class=\"favs", lastIndex)) + 1), 
				lastIndex = questData.indexOf('<', lastIndex));
	...
	public HabraQuest parseFullQuest() {
		...
		lastIndex += 17;
		quest.tags = new String(mData.substring(lastIndex,
				lastIndex = mData.indexOf("</ul>", lastIndex)));   	
		...
		String rs = mData.substring(lastIndex = (mData.indexOf('>', 
		...
		String favs = mData.substring(
				lastIndex = (mData.indexOf('>', mData.indexOf("<div class=\"favs", 
						lastIndex)) + 1), 
				lastIndex = mData.indexOf('<', lastIndex));
		if(favs.length() > 0) quest.favsCount = Integer.valueOf(favs);
		else quest.favsCount = 0;
		...
		if(lastIndex > 0)
		{
			String commentsData = new String(mData.substring(lastIndex, 
					mData.indexOf("<div class=\"hsublevel", lastIndex)));
		...
		if(lastIndex != 18)
		{
			quest.answerCount = Integer.valueOf(mData.substring(lastIndex, 
